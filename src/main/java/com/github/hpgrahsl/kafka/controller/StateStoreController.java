...
public class StateStoreController {
    ...
    @GetMapping("emojis/stats/topN/{n}")
    public Mono<Set<EmojiCount>> getEmojisTopN(@PathVariable int n) {
        return service.queryEmojiCountsTopN()
                .map(ts -> ts.stream().limit(n).collect(Collectors.toCollection(TreeSet::new)));
    }
